...
//   },[lang])
  return !use?(
    ...
    <div className=' flex flex-col gap-2 w-fit px-4 justify-center items-center'>
      <p className=' text-xl font-medium'>Microphone: {listening ? 'on' : 'off'}</p>
      <div className=' flex flex-row gap-4 '>
      <button className=' py-1 px-3 bg-slate-100 border-2 border-slate-100 text-orange-600 hover:text-slate-100 hover:bg-orange-600 rounded-2xl transition-all' onClick={()=>{listening?SpeechRecognition.stopListening():listenContinuously()}}>{listening?"Stop":"Start"}</button>
